...
Definition filter_even_gt7 (l : list nat) : list nat :=
  (filter (fun (x : nat) => (andb (ble_nat 7 x) (evenb x))) l).
...
Proof.
    ...
    Case "l = h :: t".
        simpl.
        assert (forall (f : X -> Y) (l : list X) (h : X), map f (snoc l h) = snoc (map f l) (f h)).
        SCase "Assert".
            intros f0 l h0.
...
(* This function takes one argument whose type is nat and
   compares it with 3 and if same return false (outer override)
   and if different, compares it with 1 and if same return false (inner override)
   else return ftrue(always true). *)
Check ftrue.
...
Proof.
    ...
    Case "l = h :: t".
        simpl.
        assert (forall {X : Type} (h : X) (l : list X), fold_length (h :: l) = S (fold_length l)).
        SCase "Assert".
            intros X0 h0 l.
            unfold fold_length.
            simpl.
...
Eval compute in (fold_map (fun (n : nat) => n + 1) [1; 2; 3; 4; 5]).
